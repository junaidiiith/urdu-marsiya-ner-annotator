...
from ner_annotator.constants import MAX_CONCURRENT_REQUESTS
...
def save_ner_tags(text, ner_tags, suffix=".json"):
    text_hash = calculate_hash(text)
    with open(f"{UPLOAD_DIR}/{text_hash}{suffix}", "r") as f:
        data = json.load(f)
        data["tagged_elements"] = ner_tags
        data["tagged"] = True

    with open(f"{UPLOAD_DIR}/{text_hash}{suffix}", "w") as f:
        json.dump(data, f, indent=4)
                ...
                ner_tags.append(
                    {
                        ...
                        "User Corrected": status["user_updated"]
                        if status["user_updated"] is not None
                        else "NA",
                    }
                )
...
def get_stats(tagged_data_elements):
    ...
    def get_predicted_verified(e):
        return e["tag"] if e["user_updated"] is None else "N/A"

    entity_status = [d["entity_status"] for d in tagged_data_elements]
    total_entities = [
        v for es in entity_status for k, v in es.items() if k != "user_verified"
    ]
    per_category_count = dict(
        Counter(
            [
                t["user_updated"] if t["user_updated"] else t["tag"]
                for t in total_entities
            ]
        )
    )
    # remove none counts
    ...
    total_verified = sum(
        [
            ...
            for es in entity_status
            if es.get("user_verified", False)
        ],
        [],
    )
    ...
    print(
        f"  Macro Precision: {precision_score(y_t, y_p, zero_division=0, labels=list(per_category_count.keys()), average='macro'):.3f}"
    )
    print(
        f"  Recall:    {recall_score(y_t, y_p, zero_division=0, labels=list(per_category_count.keys()), average='macro'):.3f}"
    )
    print(
        f"  F1-score:  {f1_score(y_t, y_p, zero_division=0, labels=list(per_category_count.keys()), average='macro'):.3f}\n"
    )
...
def get_llm_judgment_stats(responses_data, threshold=None):
    ...
    def compute_overall_accuracy(data, threshold=None, filters=None):
        total_accuracies = list(
            compute_entity_accuracy(entity_data, threshold, filters=filters)
            for entity_data in data.values()
        )
        # print(total_accuracies)
        total_accuracies = [
            accuracy for accuracy in total_accuracies if accuracy is not None
        ]
        # print(total_accuracies)
    ...
    def compute_entity_accuracy_per_model(data, threshold=None):
        ...
        entity_accuracy_per_model = {
            model: compute_overall_accuracy(
                data, threshold=threshold, filters={"model": model}
            )
            for model in models
        }
    ...
    def compute_accuracy_per_entity_type(data, threshold=None):
        ...
        accuracy_per_type = {
            entity_type: compute_overall_accuracy(
                data, threshold=threshold, filters={"tag": entity_type}
            )
        ...
        for tag_type in entity_types:
            for model in models:
                accuracy_per_type_per_model[tag_type][model] = compute_overall_accuracy(
                    data, threshold=threshold, filters={"tag": tag_type, "model": model}
                )
        return accuracy_per_type_per_model

    return {
        "overall_accuracy": compute_overall_accuracy(entity_predictions, threshold),
        "model_accuracy": compute_entity_accuracy_per_model(
            entity_predictions, threshold
        ),
        "entity_type_accuracy": compute_accuracy_per_entity_type(
            entity_predictions, threshold
        ),
        "model_entity_type_accuracy": compute_accuracy_per_type_per_model(
            entity_predictions, threshold
        ),
    }
...
def get_openai_response(chunked_message, model, response_format):
    try:
        response = openai.OpenAI(api_key=os.getenv("OPENAI_API_KEY"))\
            .chat.completions.parse(
            model=model,
            messages=chunked_message,
            reasoning_effort="high" if model in ["o4-mini", "o3-mini"] else NOT_GIVEN,
...
def get_deepseek_response(chunked_message, model, response_format=None):
...
def get_crewai_response(chunked_message, model, response_format):
    llm = LLM(model=model, response_format=response_format)
    try:
        response = llm.call(chunked_message)
        return response
    except Exception as e:
        print(f"Error processing chunk: {e}")
        return None
...
def get_chunks_response(chunked_messages, model, response_format, tqdm=tqdm):
    if model in OPENAI_MODELS:
        handler = get_openai_response
    elif model in CLAUDE_MODELS:
        model = "anthropic/" + model
        handler = get_crewai_response
    elif model in DEEPSEEK_MODELS:
        handler = get_deepseek_response
    else:
        raise ValueError(f"Unsupported model: {model}")
    
    extracted_results = [None] * len(chunked_messages)

    with concurrent.futures.ThreadPoolExecutor(
        max_workers=MAX_CONCURRENT_REQUESTS
    ) as executor:
        futures = {
            executor.submit(handler, chunk, model, response_format): idx 
